...
- Uu tien phong co trang thai AVAILABLE neu khach muon dat ngay.
...
function pickSuggestedRooms(message: string, rooms: Room[]): Room[] {
  ...
  return rooms
    .filter((room) => {
      ...
      const available = isAvailableStatus(room.status);
...
function extractBudget(message: string): number | null {
  const kMatch = message.match(/(?:duoi|<=|<)\s*(\d+)\s*k/);
  if (kMatch?.[1]) {
    return Number(kMatch[1]) * 1000;
  }

  const numberMatch = message.match(/(?:duoi|<=|<)\s*(\d{5,})/);
  if (numberMatch?.[1]) {
...
function extractPeople(message: string): number | null {
  const match = message.match(/(\d+)\s*(nguoi|ban|member|thanh vien)/);
...
function isAvailableStatus(status: string | null | undefined): boolean {
  ...
  return normalizedStatus === "AVAILABLE" || normalizedStatus === "TRONG";
}
