        ...
        <div className="relative mx-auto grid max-w-7xl gap-8 px-5 py-20 sm:px-8 lg:grid-cols-[1fr_420px] lg:items-end">
          ...
          <div className="rounded-2xl border border-white/12 bg-white/8 p-5 shadow-[0_20px_60px_rgba(0,0,0,0.22)]">
            ...
            <div className="mt-4 grid grid-cols-3 gap-3 text-center">
              <Metric value={String(rooms.filter((room) => room.availabilityStatus === 'AVAILABLE').length)} label="Còn trống" />
              <Metric value={String(rooms.filter((room) => room.availabilityStatus === 'ALMOST_FULL').length)} label="Sắp kín" />
              <Metric value={String(rooms.filter((room) => room.availabilityStatus === 'FULL_TODAY').length)} label="Kín lịch" />
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-5 py-10 sm:px-8">
        <div className="rounded-3xl border border-outline-variant bg-white p-5 shadow-[var(--shadow-card)]">
          <div className="grid gap-3 lg:grid-cols-[1.4fr_repeat(4,1fr)]">
            <label className="block">
              <span className="mb-2 block font-display text-xs font-bold uppercase text-on-surface-variant">Tìm phòng</span>
              <input
        ...
        <div className="mt-8 flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
          <div>
            ...
            <p className="mt-1 text-sm text-on-surface-variant">Search/filter đang chạy bằng local state, sẵn sàng thay bằng API.</p>
          </div>
          <button
            type="button"
            onClick={() => setFilters({ search: '', category: 'all', capacity: 'all', availability: 'all', price: 'all' })}
...
function RoomCard({ room }: { room: Room }) {
  const isFull = room.availabilityStatus === 'FULL_TODAY'
  ...
  return (
    <article className="group overflow-hidden rounded-3xl border border-outline-variant bg-white shadow-[var(--shadow-card)] transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_18px_48px_rgba(26,28,30,0.12)]">
      <div className="relative aspect-[16/10] overflow-hidden bg-surface-container">
        ...
        <span className={['absolute left-4 top-4 rounded-full border px-3 py-1 font-display text-xs font-bold', getAvailabilityClassName(room.availabilityStatus)].join(' ')}>
          {getAvailabilityLabel(room.availabilityStatus)}
        </span>
      ...
      <div className="p-6">
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          {room.equipments.slice(0, 3).map((equipment) => (
            <span key={equipment} className="rounded-full border border-outline-variant bg-surface-container-low px-3 py-1 text-xs font-medium text-on-surface-variant">
              {equipment}
            </span>
          ))}
        </div>
