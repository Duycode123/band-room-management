-- =============================================================
-- BandLab - Script setup database & tao cac bang ban dau
-- Database muc tieu: bandlabdb (PostgreSQL)
-- Cach chay:
--   psql -h 127.0.0.1 -p 6789 -U <user> -d bandlabdb -f BandLab_Database.sql
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'loai_thiet_bi') THEN
    CREATE TYPE "loai_thiet_bi" AS ENUM (
      'AMP', 'MIXER', 'MIC', 'TRONG', 'GUITAR', 'KEYBOARD', 'KHAC'
    );
  END IF;
...
CREATE TABLE IF NOT EXISTS "hang_phong" (
  ...
  CONSTRAINT "chk_hang_phong_suc_chua_duong" CHECK ("suc_chua" IS NULL OR "suc_chua" > 0)
);
